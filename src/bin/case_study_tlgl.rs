...
fn case_study_part_1() {
    ...
    println!("Model has {} symbolic parameters.", graph.symbolic_context().num_parameter_variables());
    println!("----------");
    ...
    println!("There are {} candidates without programmed cell death attractor, such as:\n", colors_not_pcd.approx_cardinality());
    print!("{}", graph.pick_witness(&colors_not_pcd).to_bnet(false).unwrap());
    println!("----------");
    ...
    println!("There are {} candidates with unwanted states in attractors, such as:\n", colors_with_unwanted_states.approx_cardinality());
    print!("{}", graph.pick_witness(&colors_with_unwanted_states).to_bnet(false).unwrap());
    println!("----------");
...
fn case_study_part_2(summarize_candidates: bool) {
    ...
    print!("{}", graph.pick_witness(&inferred_colors).to_bnet(false).unwrap());
    println!("----------");
